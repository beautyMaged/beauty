...
<style>
    ...
    .all_cats_list {
        {{ session('direction') == 'rtl' ? 'right:-20px!important' : '' }}
    }

    .owl-carousel.owl-rtl .owl-item {
        {{ session('direction') == 'rtl' ? 'float:right!important' : 'float:left!important' }}
    }

    #categories_list .owl-nav button.owl-prev {
        {{ session('direction') == 'rtl' ? '' : 'right:auto!important;left: 0!important' }}
    }

    #categories_list .owl-nav button.owl-next {
        {{ session('direction') == 'rtl' ? '' : 'left:auto!important;right: 0!important;' }}
    }
...
<header class="box-shadow-sm rtl __inline-10" style="z-index: 999; position:relative;">
    ...
    <div class="new_topbar text-center" dir="{{ session('direction') }}" style="background: #fcfcfd">
        ...
        <div class="row">
            @if (auth('customer')->check())
                <div
                    class="col-xxl-3 col-xl-3 col-md-3 col-sm-6 col-6 pt-3 pr-2 position-relative text-right deliverly_to">
                    <div class="dropdown">
                        <a class="navbar-tool ml-lg-3" type="button" data-toggle="dropdown"
                            ...
                            <div class="navbar-tool-text mr-3 pt-1" style="display: inline-block">
                                <span>{{ \App\CPU\translate('Welcome') }}, <span
                                        class="bold">{{ auth('customer')->user()->f_name }}</span></span>
                            </div>
                        </a>
                        <div class="dropdown-menu text-{{ session('direction') == 'rtl' ? 'right' : 'left' }}"
                    ...
                    <div class="list-ship position-relative pc_hide mob_inline" data-toggle="modal"
                        data-target="#location_modal" style="padding: 5px 0;">
                        <i class="fa-solid fa-location-dot px-1 primary_color heartbeat"
                            style="margin-top:-15px; font-size:22px;"></i>
                    ...
                    <div class="d-inline-block pc_hide mob_inline" style="padding-right: 11px;">
                        @foreach (json_decode($lang['value'], true) as $data)
                            @if ($data['code'] != $local)
                                <a href="{{ route('lang', $data['code']) }}"><span class="bold"
                                        style="font-size:12px;margin-right: 0px;cursor: pointer">{{ \App\CPU\translate($data['name']) }}</span>
                                </a>
                            @endif
                        @endforeach
                    </div>
                ...
                <div class="col-sm-6 col-6 pt-2 text-left pc_hide">
                    <div class="navbar-toolbar shorthand_icons  text-center pc_hide" dir="rtl"
                        ...
                        @if ($currency_model == 'multi_currency')
                            <div class="topbar-text dropdown disable-autohide d-inline-block position-relative">
                                <a class="topbar-link dropdown-toggle" href="#" data-toggle="dropdown">
                                    <span> {{ session('currency_symbol') }} <i class="fa-solid fa-globe"
                        ...
                        <div class="navbar-tool d-inline-block mr-lg-1 position-relative dropdown">
                            <a class="navbar-tool-icon-box bg-secondary dropdown-toggle"
                                ...
                                <span class="navbar-tool-label">
                                    @if (auth('customer')->check())
                                        <span
                                            class="countWishlist">{{ \App\Model\Wishlist::where('customer_id', auth('customer')->id())->count() }}</span>
                                    @else
                                        <span
            ...
            @else
                <div
                    class="col-xxl-3 col-xl-3 col-md-3 col-sm-6 col-6 pt-4 position-relative {{ session('direction') == 'rtl' ? 'text-right pr-3 ' : 'text-left  pl-4 ' }} deliverly_to">
                    <span type="button" data-toggle="modal" data-target="#register_modal"
                        class="mobile_hide reg_aa" style="cursor: pointer"> <i
                            class="fa-regular fa-user px-1 s_19 primary_color"></i> <span class="s_18 bold"><span
                                class="mobile_hide">{{ \App\CPU\translate('You are a Guest / ') }}</span>
                            {{ \App\CPU\translate('Sign in') }}</span> </span>
                    <div class="list-ship position-relative pc_hide mob_inline" data-toggle="modal"
                        data-target="#location_modal" style="padding: 5px 0;">
                        <i class="fa-solid fa-location-dot px-1 primary_color heartbeat"
                            style="margin-top:-15px; font-size:22px;"></i>
                    ...
                    <div class="d-inline-block pc_hide mob_inline" style="    padding-right: 11px;">
                        @foreach (json_decode($lang['value'], true) as $data)
                            @if ($data['code'] != $local)
                                <a href="{{ route('lang', $data['code']) }}"><span class="bold"
                                        style="font-size:12px;margin-right: 9px;cursor: pointer">{{ \App\CPU\translate($data['name']) }}</span></a>
                            @endif
                        @endforeach
                    </div>
                </div>
                <div class="col-sm-6 col-6 pt-2 text-left pc_hide">
                    <div class="navbar-toolbar shorthand_icons  text-center pc_hide" dir="rtl"
                        ...
                        <div class="d-inline-block position-relative" style="margin-right: 2px!important;"><span
                                type="button" data-toggle="modal" data-target="#register_modal"
                                style="cursor: pointer"> <i class="fa-regular fa-user s_19 primary_color"></i> <span
                                    class="s_12 bold"><span
                                        class="mobile_hide">{{ \App\CPU\translate('You are a Guest / ') }}</span>
                                    {{ \App\CPU\translate('Sign in') }}</span> </span> </div>
                        <div id="cart_items_mobile" class="d-inline-block mr-lg-2 position-relative">
                            @include('layouts.front-end.partials.cart')
                        </div>
                        <div class="navbar-tool d-inline-block mr-lg-1 position-relative dropdown">
                        ...
                        <div class="navbar-tool d-inline-block mr-lg-1 position-relative dropdown">
                            <a class="navbar-tool-icon-box bg-secondary dropdown-toggle"
                                href="tel:{{ $cs_phone->value }}"> <i class="fa-solid fa-headset px-1 s_19"></i>
                                {{-- <span class="s_12 bold mobile_hide">خدمة العملاء</span> --}} </a>
                        </div>
                    </div>
                </div>
            @endif
            @php($whatsapp = \App\CPU\Helpers::get_business_settings('whatsapp'))
            <div class=" col-sm-6 col-6 pt-2 text-left social_icons_mobile d-none"
                style="padding-left: 23px; display: none">
                <a href="https://web.whatsapp.com/send/?phone={{ $whatsapp['phone'] }}?text=Hello%20there!"
                    class="px-1"><img src="{{ asset('assets/front-end/img/whatsapp.png') }}" alt="icon"
                        style="width: 27px"> </a>
                <a href="{{ App\Model\SocialMedia::where('name', 'twitter')->first()->link }}" class="px-1"> <i
                        class="fa-brands fa-twitter"></i> </a>
                <a href="{{ App\Model\SocialMedia::where('name', 'instagram')->first()->link }}" class="px-1"> <i
                        class="fa-brands fa-instagram"></i></a>
                <a href="{{ App\Model\SocialMedia::where('name', 'facebook')->first()->link }}" class="px-1"> <i
            ...
            <div class="col-xxl-6 col-xl-6 col-md-6 col-sm-12 col-12 bold pt-2 mobile_hide">
                <a class="navbar-brand d-none d-sm-block {{ Session::get('direction') === 'rtl' ? 'mr-3' : 'mr-3' }} flex-shrink-0 __min-w-7rem"
                    href="{{ route('home') }}">
                    <img class="__inline-11" src="{{ asset('storage/company') . '/' . $web_config['web_logo']->value }}"
                        style="height: 50px!important;"
                        onerror="this.onerror=null;this.src='{{ asset('assets/front-end/img/image-place-holder.png') }}'"
                        alt="{{ $web_config['name']->value }}" />
                </a>
            </div>
            <div class="col-xxl-3 col-xl-3 col-md-3 col-sm-12 col-12 {{ session('direction') == 'rtl' ? 'text-left' : 'text-right' }} social_icons mobile_hide"
                style=" {{ session('direction') == 'rtl' ? 'padding-left: 23px;' : 'padding-right: 23px;' }} padding-top: 20px">
                <a href="https://web.whatsapp.com/send/?phone={{ $whatsapp['phone'] }}?text=Hello%20there!"
                    target="_blank" class="px-1"> <img src="{{ asset('assets/front-end/img/whatsapp.png') }}"
                        alt="icon" style="width: 27px"> </a>
                <a href="{{ App\Model\SocialMedia::where('name', 'facebook')->first()->link }}" class="px-1"> <img
                        src="{{ asset('assets/front-end/img/fb.png') }}" alt="icon" style="width: 23px"> </a>
                <a href="{{ App\Model\SocialMedia::where('name', 'linkedin')->first()->link }}" class="px-1"> <img
                        src="{{ asset('assets/front-end/img/linkedin.png') }}" alt="icon" style="width: 23px">
                </a>
                <a href="{{ App\Model\SocialMedia::where('name', 'instagram')->first()->link }}" class="px-1"> <img
                        src="{{ asset('assets/front-end/img/instagram.png') }}" alt="icon" style="width: 23px">
                </a>
                <a href="{{ App\Model\SocialMedia::where('name', 'twitter')->first()->link }}" class="px-1"> <img
                        src="{{ asset('assets/front-end/img/twitter.png') }}" alt="icon" style="width: 23px">
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="navbar navbar-expand-md navbar-light">
            <div class="container nav_2 row m-auto" dir="{{ session('direction') }}">
                ...
                <div class="col-lg-4 col-md-4 mobile_hide pr-0">
                    ...
                    <div class="list-ship position-relative " data-toggle="modal" data-target="#location_modal"
                        style="width: auto;padding: 5px 0 0 5px;">
                        <i class="fa-solid fa-location-dot pl-1 pr-0 primary_color heartbeat"
                            style="margin-top:-15px; font-size:22px;"></i>
                        ...
                        <div class="shipping-list-items" style="padding: 15px">
                            ...
                            <div class="input-group mt-2" dir="ltr">
                                <div class="input-group-prepend">
                                    <span class="input-group-text" id="basic-addon1" style="padding: 4px"
                                        data-toggle="modal" data-target="#location_modal"> <img
                                            src="{{ asset('assets/front-end/img/send.png') }}" alt="send"
                                            style="width: 21px"> </span>
                                </div>
                                <input type="text" class="form-control" placeholder="اسم المنطقة"
                                    style="padding: 6px 7px 0 7px!important;height: 33px;text-align: right;">
                            </div>
                        </div>
                    </div>
                    <div class="d-inline-block">
                        @foreach (json_decode($lang['value'], true) as $data)
                            @if ($data['code'] != $local)
                                <a href="{{ route('lang', $data['code']) }}"><span class="s_19 bold "
                                        style="margin-right: 0px;cursor: pointer">{{ \App\CPU\translate($data['name']) }}</span>
                                    <i class="fa-solid fa-globe" style="font-size: 16px"></i></a>
                            @endif
                        @endforeach
                    </div>
                </div>
                <a class="navbar-brand mobile_logo d-sm-none {{ Session::get('direction') === 'rtl' ? 'mr-2' : 'mr-2' }}"
                    href="{{ route('home') }}"> <img class="mobile-logo-img __inline-12"
                        src="{{ asset('storage/company') . '/' . $web_config['web_logo']->value }}"
                        onerror="this.onerror=null;this.src='{{ asset('assets/front-end/img/image-place-holder.png') }}'"
                        alt="{{ $web_config['name']->value }}" /> </a>
                <!-- Search-->
                <div class="input-group-overlay d-none d-md-inline-block col-lg-4 col-md-4 mx-4"
                    style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }}">
                    <form action="{{ route('products') }}" type="submit" class="search_form">
                        <input class="form-control appended-form-control search-bar-input" type="text"
                            autocomplete="off" dir="rtl"
                            placeholder="{{ \App\CPU\translate('Search in Store') }} ..." name="name">
                        <button class="input-group-append-overlay search_button secondary_bg" type="submit"
                            style="border-radius: 5px 0 0 5px!important; right: unset; left: -6px;top:0;"> <span
                                class="input-group-text __text-20px"> <i class="czi-search text-white"></i> </span>
                        </button>
                ...
                <!-- Toolbar-->
                <div class="navbar-toolbar shorthand_icons col-lg-4 col-md-4 pt-4 pb-2 mobile_hide"
                    dir="{{ session('direction') }}"
                    style="{{ session('direction') == 'rtl' ? 'text-align: left;margin-right: -44px;' : 'text-align: right;margin-left: -44px;' }}">
                    @php($currency_model = \App\CPU\Helpers::get_business_settings('currency_model'))
                    @if ($currency_model == 'multi_currency')
                        <div class="topbar-text dropdown disable-autohide d-inline-block position-relative">
                            <a class="topbar-link dropdown-toggle" href="#" data-toggle="dropdown"> <span>
                                    {{ session('currency_symbol') }} </span> </a>
                            <ul class="dropdown-menu dropdown-menu-{{ Session::get('direction') === 'rtl' ? 'right' : 'left' }}"
                                style="min-width: 160px!important;text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
                                @foreach (\App\Model\Currency::where('status', 1)->get() as $key => $currency)
                                    <li class="dropdown-item cursor-pointer"
                                        onclick="currency_change('{{ $currency['code'] }}')"> {{ $currency->name }}
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <div class="navbar-tool d-inline-block mr-1 position-relative dropdown">
                        <a class="navbar-tool-icon-box bg-secondary dropdown-toggle"
                            href="tel:{{ $cs_phone->value }}"> <i class="fa-solid fa-headset px-1 "
                                style="font-size: 20px!important;"></i> {{-- <span class="s_12 bold mobile_hide">خدمة العملاء</span> --}} </a>
                    </div>
                </div>
            </div>
            ...
            <div class="m-auto col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="row pb-0 categories_container position-relative" dir="rtl">
                    @if (session('direction') == 'rtl')
                        <div class="col-lg-2 col-md-2 col-sm-2 col-2 {{ session('direction') == 'rtl' ? 'text-right' : 'text-left' }}  all_cats_div"
                            style="padding: 7px">
                            <a class="all_cats_btn s_16 bold " href="#"
                                style="{{ session('direction') == 'rtl' ? '' : 'padding: 7px 0 7px 18px;' }}">
                                {{ \App\CPU\translate('All Categories_') }} <i
                                    class="fa fa-caret-down position-absolute"
                                    style="top: 16px; {{ session('direction') == 'rtl' ? 'left:50px;' : 'right:50px;' }} color: #fff"
                                    aria-hidden="true"></i> </a>
                        </div>
                    @endif
                    <div class="col-lg-10 col-md-10 col-sm-10 col-10">
                        <div class="categories_list pr-0 py-1 my-0 text-right" dir="ltr">
                            <div class="owl-carousel owl-theme " id="categories_list"
                                style="padding-left: 2.5rem!important; padding-right: 2.5rem!important;">
                                <div style="" class="category-item_1"> <a
                                        href="{{ route('home') }}/products?data_from=best-selling&page=1"><span
                                            class="boldy">{{ \App\CPU\translate('top_sell_pro') }}</span></a> </div>
                                <div style="" class="category-item_1"> <a
                                        href="{{ route('home') }}/products?data_from=top-rated&page=1"><span
                                            class="boldy">{{ \App\CPU\translate('top_rate_pro') }}</span></a> </div>
                                <div style="" class="category-item_1"> <a
                                        href="{{ route('home') }}/flash-deals/1"><span
                                            class="boldy">{{ \App\CPU\translate('daily_offers') }}</span></a> </div>
                                <div style="" class="category-item_1"> <a
                                        href="{{ route('home') }}/products?data_from=latest&page=1"><span
                                            class="boldy">{{ \App\CPU\translate('recent_pro') }}</span></a> </div>
                                <div style="" class="category-item_1"> <a
                                        href="{{ route('products', ['data_from' => 'featured_deal', 'page' => 1]) }}"><span
                                            class="boldy">{{ \App\CPU\translate('special_offers') }}</span></a>
                                </div>
                                @php(
    $cats = $all_cats->filter(fn($item) => $item->position == 0 && $item->home_status == true)->sortBy('priority')->values(),
)
                                @foreach ($cats as $cat)
                                    <div style="" class="category-item"
                                        data-value="menu_cat_{{ $cat->id }}" data-id="{{ $cat->id }}">
                                        <a
                                            href="{{ route('home') }}/products?id={{ $cat->id }}&data_from=category&page=1"><span
                                                class="boldy">{{ $cat->name }}</span></a> </div>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    @if (session('direction') == 'ltr')
                        <div class="col-xxl-2 col-xl-2 col-md-2 col-sm-2 col-2 {{ session('direction') == 'rtl' ? 'text-right' : 'text-left' }}  all_cats_div"
                            style="padding: 7px">
                            <a class="all_cats_btn s_16 bold " href="#"
                                style="{{ session('direction') == 'rtl' ? '' : 'padding: 7px 0 7px 18px;' }}">
                                {{ \App\CPU\translate('All Categories_') }} <i
                                    class="fa fa-caret-down position-absolute"
                                    style="top: 16px; {{ session('direction') == 'rtl' ? 'left:20px;' : 'right:20px;' }} color: #f14061"
                                    aria-hidden="true"></i> </a>
                        </div>
                    @endif
                    @foreach ($cats as $cat)
                        <div class="sub-categories-list col-lg-12 col-md-12 col-sm-12 col-12"
                            id="menu_cat_{{ $cat->id }}">
                            <div class="row ">
                                <div class="col-xxl-3 col-xl-3 col-md-3 col-sm-3 col-3" style="padding: 22px;">
                                    <img src="{{ asset('storage/category/' . $cat->icon) }}" alt=""
                                        style="width:300px; height:300px;border-radius: 5px;box-shadow: 3px 2px 2px 1px #eadcdc33;">
                                </div>
                                <div class="col-xxl-2 col-xl-2 col-md-2 col-sm-2 col-2"
                                    style="padding: 22px 17px 0 0;">
                                    <div class="row">
                                        @php(
    $catChildes = $all_cats->filter(function ($item) use ($cat) {
            return $item->position == 1 && $item->parent_id == $cat->id;
                                        ...
                                        <div class="{{ $catChildes->count() > 0 ? 'col-xxl-5 col-xl-5 col-lg-5 col-md-5 col-sm-5 col-12' : 'col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 text-center' }}"
                                            style="padding: 74px 0px;border-left: 1px solid #ddd;">
                                            <a href="{{ route('home') }}/products?id={{ $cat->id }}&data_from=category&page=1"
                                                class="bold s_16 d-block"
                                                style="padding: 13px 0;">{{ \App\CPU\translate('View All') }}</a>
                                            <a href="{{ route('home') }}/products?id={{ $cat->id }}&data_from=top-rated&page=1"
                                                class="bold s_16 d-block"style="padding: 13px 0;">{{ \App\CPU\translate('Top Rated') }}</a>
                                        </div>
                                        @if ($catChildes->count() > 0)
                                            <div class="col-xxl-7 col-xl-7 col-lg-7 col-md-7 col-sm-7 col-12 s_14"
                                                style="padding-right: 14px;border-left: 1px solid #ddd;">
                                                ...
                                                <ul style="list-style: none;padding-right: 11px;">
                                                    @foreach ($catChildes as $single_child)
                                                        <li style="padding: 4px 0;"><a
                                                                href="{{ route('home') }}/products?id={{ $single_child->id }}&data_from=category&page=1">{{ $single_child->name }}</a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            </div>
                                ...
                                @php(
    $brands_ids = $porduct_data->whereIn('id', $product_ids)->pluck('brand_id')->toArray(),
)
                                @php(
    $brands = App\Model\Brand::whereIn('id', $brands_ids)->limit(6)->get(),
)
                                @if ($colors->count() > 0)
                                    <div class="col-xxl-1 col-xl-1 col-md-1 col-sm-1 col-1 text-center"
                                        style="padding: 22px 17px 0 0;">
                                        <div class="row">
                                            <div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 s_14"
                                                style="padding-left: 20px;border-left: 1px solid #ddd;">
                                                <h3 class="bold s_14">{{ \App\CPU\translate('Shop With Color') }}
                                                </h3>
                                                <div>
                                                    @foreach ($colors as $color)
                                                        <a
                                                            href="{{ route('products', ['id' => $cat->id, 'data_from' => 'category', 'color' => str_replace('#', '', $color->code), 'page' => 1]) }}">
                                                            <div class="color-item-nav"
                                ...
                                @endif
                                <div class="col-xxl-3 col-xl-3 col-md-3 col-sm-3 col-3"
                                    style="padding: 22px 17px 0 0;">
                                    <div class="row">

                                        <div
                                            class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 col-12 text-center">
                                            <h3 class="bold s_18">{{ \App\CPU\translate('Sub Sub Category') }}</h3>
                                        </div>
                                        {{--   @php($most_pop = \App\Model\Product::) --}}
                                        @foreach ($brands as $brand)
                                            <div
                                                class="col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-4 col-xs-4 col-4 text-center py-3">
                                                <a href="#"> <img
                                                        src="{{ asset('storage/brand/' . $brand->image) }}"
                                                        class="most_pop_prod d-block" alt=""
                                                        style="border-radius: 0;padding: 7px;width: 100%;height: 93px">
                                                    <span class="s_14 bold d-block">{{ $brand->name }}</span> </a>
                                            </div>
                                        @endforeach
                                ...
                                <div class="col-xxl-3 col-xl-3 col-md-3 col-sm-3 col-3"
                                    ...
                                    <div class="row">
                                        ...
                                        @endforeach
                                        @php(
    $most_pop = $porduct_data->whereIn('id', $product_ids)->limit(4)->get(),
)
                                        @foreach ($most_pop as $single_pop)
                                            ...
                                            <div
                                                ...
                                                <a href="{{ route('product', $single_pop->slug) }}">
                                                    <img {{--                                                        src="{{\App\CPU\ProductManager::product_image_path('thumbnail')}}/{{$single_pop['thumbnail']}}" --}}
                    ...
                    <div class="all_cats_list col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-6 colxs-6 col-6"
                        dir="{{ session('direction') }}"
                        style="{{ session('direction') == 'ltr' ? 'left:6px;' : '' }}">
                        <div class="row all_cats_row">
                            <div class="all_main_cats col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-4 col-xs-4 col-4"
                                style="border: 1px solid #ddd;">
                                <ul class="all_main_cats_list">
                                    {{-- @php($special_main_cats=\App\CPU\CategoryManager::parents()) --}}

                                    @foreach ($cats as $cat)
                                        <li class="single-main-item" data-target="{{ $cat->id }}"> <a
                                                href="{{ route('home') }}/products?id={{ $cat->id }}&data_from=category&page=1"
                                                class="s_18">{{ $cat->name }}</a> </li>
                                    @endforeach
                                </ul>
                            </div>
                            @foreach ($cats as $key_sp => $cat)
                                <div class="all_main_cats sub_cats_menu col-xxl-8 col-xl-8 col-lg-8 col-md-8 col-sm-8 col-xs-8 col-8 {{ $key_sp = '0' ? 'show' : '' }}"
                                    id="sub_items_from_main_{{ $cat->id }}">
                                    <div class="row" style="padding: 10px">
                                        ...
                                        <div class="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-6 col-6">
                                            ...
                                            <ul
                                                style="list-style: none; padding: 0px 13px;border-left: 1px solid #ddd;">
                                                @php(
    $catChildes = $all_cats->filter(function ($item) use ($cat) {
            return $item->position == 1 && $item->parent_id == $cat->id;
        })->sortBy('priority')->values(),
                                                ...
                                                @if ($catChildes->count() > 0)
                                                    @foreach ($catChildes as $single_sub)
                                                        <li class="sub-item" data-target="{{ $single_sub->id }}"> <a
                                                                href="{{ route('home') }}/products?id={{ $single_sub->id }}&data_from=category&page=1"
                                                                class="s_14">{{ $single_sub->name }}</a></li>
    ...
    $subCats = $all_cats->filter(function ($item) use ($cat) {
            return $item->position == 1 && $item->parent_id == $cat->id;
                                        ...
                                        @foreach ($subCats as $key_sub => $sub_cat)
                                            <div id="sub_items_from_sub_{{ $sub_cat->id }}"
                                                class="sub_sub_cats_menu col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-6 col-6">
                                                    ...
                                                    @if ($subCatChildes->count() > 0)
                                                        @foreach ($subCatChildes as $single_sub_sub)
                                                            <li class="sub-sub-item"
                                                                data-target="{{ $single_sub_sub->id }}"> <a
                                                                    href="{{ route('home') }}/products?id={{ $single_sub_sub->id }}&data_from=category&page=1"
                                                                    class="s_14">{{ $single_sub_sub->name }}</a>
                                                            </li>
                                                        @endforeach
                                                    @endif
                                                </ul>
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-12">
                <form action="{{ route('products') }}" type="submit" class="search_form" dir="ltr">
                    <input class="form-control appended-form-control search-bar-input" type="text"
                        autocomplete="off" dir="rtl" placeholder="ابحث ..." name="name">
                    <button class="input-group-append-overlay search_button" type="submit"
                        style="border-radius: {{ Session::get('direction') === 'rtl' ? '7px 0px 0px 7px; right: unset; left: 0' : '0px 0 7px 7px; right: unset; left: 0' }};top:0">
                            ...
                            @foreach ($categories as $category)
                                <li class="dropdown">
                                    <a
                                        ...
                                        <img src="{{ asset('assets/front-end/img/sub-1.png') }}"
                                            onerror="this.onerror=null;this.src='{{ asset('assets/front-end/img/image-place-holder.png') }}'"
                                            class="__img-18" style="margin-left: 11px;"> <span
                                            class="bold {{ Session::get('direction') === 'rtl' ? 'pr-3' : 'pl-3' }}">{{ $category->name }}</span>
                                    </a>
                                    <a class='__ml-50px expanding_btn_sub'> <i
                                            class="czi-arrow-{{ Session::get('direction') === 'rtl' ? 'right' : 'left' }} __inline-16 "></i>
                                    </a>
                                    <ul class="dropdown-menu sub_dropdown-menu"
                                        ...
                                        @foreach ($category->childes as $child)
                                            <li class="dropdown">
                                                <a
                                                    href="{{ route('products', ['id' => $child->id, 'data_from' => 'category', 'page' => 1]) }}">
                                                    <span
                                                        class="bold {{ Session::get('direction') === 'rtl' ? 'pl-3' : 'pr-3' }}">{{ $child->name }}</span>
                                                </a>
                                                <a style="font-family:  sans-serif !important;font-size: 1rem;font-weight: 300;line-height: 1.5;margin-left:50px;"
                                                    class="expanding_btn_sub_sub"> <i
                                                        class="czi-arrow-{{ Session::get('direction') === 'rtl' ? 'right' : 'left' }} __inline-16"></i>
                                                </a>
                                                <ul class="dropdown-menu sub_sub_dropdown-menu">
                                                    @foreach ($child->childes as $ch)
                                                        <li> <a class="dropdown-item bold"
                                                                href="{{ route('products', ['id' => $ch->id, 'data_from' => 'category', 'page' => 1]) }}">
                                                                {{ $ch->name }} </a> </li>
                                                    @endforeach
                                                </ul>
                                            </li>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#categories_list').owlCarousel({
                ...
                @endif
                @if (session('direction') == 'rtl')
                    navText: ["<img src='{{ asset('assets/front-end/img/chevron-right.png') }}'>",
                        "<img src='{{ asset('assets/front-end/img/chevron-left.png') }}'>"
                    ],
                @else
                    navText: ["<img src='{{ asset('assets/front-end/img/chevron-left.png') }}'>",
                        "<img src='{{ asset('assets/front-end/img/chevron-right.png') }}'>"
                    ],
                @endif
                responsive: {
                    //X-Small
