                    ...
                    if (json_decode($product->variation)[$i]->qty < 1) {
                        ...
                        if(session('direction') == 'rtl') {
                            return [
                                ...
                                'price' =>   \App\CPU\Helpers::currency_converter(json_decode($product->variation)[$i]->price) . ' ' . $usd
                            ];
                        } else {
                            return [
                                'status' => 1,
                                'message' => json_decode($product->variation)[$i]->qty  . ' peaces in stock',
                                'price' =>   \App\CPU\Helpers::currency_converter(json_decode($product->variation)[$i]->price)
                            ];
                        }
                    }
                }
